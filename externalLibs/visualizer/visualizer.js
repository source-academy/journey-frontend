(function (exports) {
  ...
  function list_to_tree(lst) {
    ...
    function construct_tree(lst) {
      ...
      } else if (is_function(head(lst))) {
        // draw the function object
        if (perms.indexOf(head(lst)) > -1) {
          // check if function has been drawn
          thisNode.leftid = perms.indexOf(head(lst));
        } else {
    ...
    if (node !== null) {
      ...
      if (node.left !== null) {
        ...
        backwardLeftEdge(
          x,
          y,
          nodelist[node.leftid].getX(),
          nodelist[node.leftid].getY(),
          layer
        );
